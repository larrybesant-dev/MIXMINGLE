...
(async () => {
  ...
  const flutterElements = await page.locator('flt-glass-pane, flt-semantics-placeholder, [data-semantic]').all();
  console.log(`Found ${flutterElements.length} Flutter elements`);

  // Check for login elements
  const loginElements = await page.locator('input[type="email"], input[type="password"], button:has-text("Sign"), button:has-text("Login")').all();
  ...
  const buttons = await page.locator('button, [role="button"], input[type="button"], a, flt-semantics-placeholder[role="button"]').all();
  ...
  const keyElements = await page.locator('[id*="key"], [class*="key"], [data-testid], [aria-label]').all();
