...
(async () => {
  ...
  try {
    ...
    const isLoading = await page
      .locator("text=/Loading|Initializing|Splash/i")
      .isVisible()
      .catch(() => false);
    if (isLoading) {
    ...
    if (isOnLoginPage) {
      ...
      const loginTitle = await page
        .locator("text=/Mix.*Mingle|Welcome|Login/i")
        .first()
        .isVisible()
        .catch(() => false);
      if (loginTitle) {
      ...
      const csvContent =
        "Feature,Status,Details,Timestamp\n" +
        results
          .map(
            (r) =>
  ...
  const csvRows = results
    .map((r) => `"${r.feature}","${r.status}","${r.details.replace(/"/g, '""')}","${r.timestamp}"`)
    .join("\n");
