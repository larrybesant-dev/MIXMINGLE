...
async function login(page) {
  ...
  await page.waitForSelector("input, .mdc-text-field__input, [data-semantics-role]", {
    timeout: 20000,
  });
...
test.describe("Mix & Mingle QA Pipeline", () => {
  ...
  test("Room creation - assert room title appears", async ({ page }) => {
    ...
    try {
      ...
      try {
        createRoomButton = page
          .locator("button")
          .filter({ hasText: /create.*room/i })
          .first();
        await createRoomButton.waitFor({ timeout: 5000 });
      } catch {
        throw new Error(
          "Create Room button not found - app may not be rendering or user not authenticated",
        );
      }
    }
  ...
  test("Participant count - assert it updates after join", async ({ page }) => {
    ...
    try {
      ...
      try {
        ...
        initialCountText = await roomCard
          .locator("text")
          .filter({ hasText: /\d+/ })
          .first()
          .textContent();
    ...
    if (updatedCount <= initialCount) {
      throw new Error(
        `Participant count did not update. Initial: ${initialCount}, Updated: ${updatedCount}`,
      );
    }

    console.log(
      "✅ Participant count test passed: Count updated from",
      initialCount,
      "to",
      updatedCount,
    );
  ...
  test("Settings - assert changes persist after reload", async ({ page }) => {
    ...
    try {
      ...
      try {
        ...
        try {
          settingsButton = page
            .locator('[data-testid*="settings"], [aria-label*="settings" i]')
            .first();
          await settingsButton.waitFor({ timeout: 5000 });
        } catch {
    ...
    if (isNowChecked === wasChecked) {
      throw new Error(
        `Settings change did not persist after reload. Was checked: ${wasChecked}, Now checked: ${isNowChecked}`,
      );
    }
