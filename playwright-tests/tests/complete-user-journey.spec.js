...
test.describe('Complete User Journey - Mix & Mingle', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    try {
      ...
      throw new Error(`App not accessible: ${error.message}. Make sure 'flutter run -d chrome --web-port=3000' is running.`);
    }
  });
  ...
  test('should complete full new user journey from signup to messaging', async ({ page }) => {
    ...
    const signInButton = page.locator('text=Sign In').or(page.locator('text=Login')).or(page.locator('text=Get Started'));
    ...
    if (isAuthRequired) {
      ...
      // Fill signup form
      const emailField = page.locator('input[type="email"]').or(page.locator('[data-testid="email"]')).or(page.locator('input[placeholder*="email" i]'));
      const passwordField = page.locator('input[type="password"]').or(page.locator('[data-testid="password"]')).or(page.locator('input[placeholder*="password" i]'));
      const confirmPasswordField = page.locator('input[placeholder*="confirm" i]').or(page.locator('input[placeholder*="repeat" i]'));
      const submitButton = page.locator('button[type="submit"]').or(page.locator('text=Sign Up')).or(page.locator('text=Create Account')).or(page.locator('text=Register'));
    ...
    if (profileSetupRequired) {
      ...
      const nameField = page.locator('input[placeholder*="name" i]').or(page.locator('[data-testid="display-name"]'));
      const usernameField = page.locator('input[placeholder*="username" i]').or(page.locator('[data-testid="username"]'));
      const bioField = page.locator('textarea').or(page.locator('input[placeholder*="bio" i]')).or(page.locator('input[placeholder*="about" i]'));
      const saveButton = page.locator('text=Save').or(page.locator('text=Continue')).or(page.locator('text=Complete')).or(page.locator('button[type="submit"]'));
    ...
    if (!onHomePage) {
      ...
      const homeButton = page.locator('text=Home').or(page.locator('[data-testid="home-btn"]')).or(page.locator('nav a').first());
      if (await homeButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        await homeButton.click();
      }
    }

    expect(onHomePage || (await page.locator('body').textContent()).includes('Welcome')).toBeTruthy();

    // === PHASE 5: BROWSE ROOMS ===
    console.log('🏠 Phase 5: Browse Rooms');

    const roomsButton = page.locator('text=Browse Rooms').or(page.locator('text=Rooms')).or(page.locator('[data-testid="browse-rooms-btn"]'));
    if (await roomsButton.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      if (roomsPageLoaded) {
        ...
        const createRoomButton = page.locator('text=Create Room').or(page.locator('text=New Room')).or(page.locator('[data-testid="create-room-btn"]'));
        if (await createRoomButton.isVisible({ timeout: 2000 }).catch(() => false)) {
          await createRoomButton.click();

          // Should show "coming soon" snackbar
          const snackbar = page.locator('text=coming soon').or(page.locator('text=Coming Soon')).or(page.locator('text=feature coming soon'));
    ...
    const usersButton = page.locator('text=Discover Users').or(page.locator('text=Users')).or(page.locator('[data-testid="discover-users-btn"]'));
    if (await usersButton.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      if (usersPageLoaded) {
        ...
        const userCard = page.locator('[data-testid="user-card"]').or(page.locator('.user-card')).or(page.locator('text=Follow').first());
    ...
    const messagesButton = page.locator('text=Messages').or(page.locator('text=Chat')).or(page.locator('[data-testid="messages-btn"]'));
    if (await messagesButton.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      if (messagesPageLoaded) {
        ...
        const newMessageButton = page.locator('text=New Message').or(page.locator('text=Start Chat')).or(page.locator('[data-testid="new-message-btn"]'));
    ...
    const settingsButton = page.locator('text=Settings').or(page.locator('text=Preferences')).or(page.locator('[data-testid="settings-btn"]'));
    if (await settingsButton.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      if (settingsPageLoaded) {
        ...
        const settingToggle = page.locator('input[type="checkbox"]').or(page.locator('button')).first();
    ...
    const profileButton = page.locator('text=Profile').or(page.locator('text=My Profile')).or(page.locator('[data-testid="profile-btn"]'));
    if (await profileButton.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      if (profilePageLoaded) {
        ...
        const editButton = page.locator('text=Edit').or(page.locator('text=Edit Profile')).or(page.locator('[data-testid="edit-profile-btn"]'));
  ...
  test('should handle authentication errors gracefully', async ({ page }) => {
    ...
    if (await signInButton.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      if (await emailField.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        const errorMessage = page.locator('text=error').or(page.locator('text=Error')).or(page.locator('text=coming soon'));
