...
final schemaFriendLinksProvider = StreamProvider.autoDispose<List<SchemaFriendLink>>((ref) {
...
final schemaAcceptedFriendLinksProvider = Provider.autoDispose<List<SchemaFriendLink>>((ref) {
  final links = ref.watch(schemaFriendLinksProvider).valueOrNull ?? const <SchemaFriendLink>[];
  return links.where((link) => link.isAccepted).toList(growable: false);
});

final schemaAcceptedFriendIdsProvider = Provider.autoDispose<List<String>>((ref) {
  final userId = ref.watch(schemaAuthUserIdProvider).value;
...
final schemaIncomingFriendRequestsProvider = Provider.autoDispose<List<SchemaFriendLink>>((ref) {
  final userId = ref.watch(schemaAuthUserIdProvider).value;
  final links = ref.watch(schemaFriendLinksProvider).valueOrNull ?? const <SchemaFriendLink>[];
...
final schemaOutgoingFriendRequestsProvider = Provider.autoDispose<List<SchemaFriendLink>>((ref) {
  final userId = ref.watch(schemaAuthUserIdProvider).value;
  final links = ref.watch(schemaFriendLinksProvider).valueOrNull ?? const <SchemaFriendLink>[];
  if (userId == null || userId.isEmpty) return const <SchemaFriendLink>[];
...
final schemaFriendIdentityProvider =
    StreamProvider.autoDispose.family<SchemaFriendIdentity?, String>((ref, friendId) {
  final firestore = ref.watch(schemaFriendFirestoreProvider);
  ...
  return Stream.multi((controller) {
    ...
    userSub = firestore.collection('users').doc(friendId).snapshots().listen((snapshot) {
      if (disposed || controller.isClosed) return;
      if (!snapshot.exists) {
...
final schemaFriendPresenceProvider =
  ...
  return firestore
      ...
      .snapshots()
      .map((snapshot) => SchemaFriendPresence.fromParticipantDocs(friendId, snapshot.docs));
});
...
final schemaFriendPresenceMapProvider =
  ...
  List<List<String>> chunkIds(List<String> ids) {
    ...
    for (var index = 0; index < ids.length; index += whereInLimit) {
      final end = (index + whereInLimit) > ids.length ? ids.length : index + whereInLimit;
    ...
    void emit() {
      ...
      controller.add({
        for (final friendId in friendIds)
          friendId: merged[friendId] ?? SchemaFriendPresence.offline(friendId),
      });
    }
      ...
      final sub = firestore
        ...
        final presenceMap = <String, SchemaFriendPresence>{
          for (final friendId in idsChunk)
            friendId: SchemaFriendPresence.fromParticipantDocs(friendId, grouped[friendId] ?? const []),
        };
...
class SchemaFriendLinksController {
  SchemaFriendLinksController(this._firestore);
  ...
  Future<void> sendRequest({
    ...
    final normalizedFrom = fromUserId.trim();
    final normalizedTo = toUserId.trim();
    if (normalizedFrom.isEmpty || normalizedTo.isEmpty || normalizedFrom == normalizedTo) {
      return;
    }

    final docId = SchemaFriendLink.canonicalIdFor(normalizedFrom, normalizedTo);
    final users = SchemaFriendLink.sortedUsers(normalizedFrom, normalizedTo);

    await _firestore.collection('friend_links').doc(docId).set({
      'users': users,
      'status': 'pending',
      'requestedBy': normalizedFrom,
  ...
  Future<void> acceptRequest({required String linkId}) async {
    await _firestore.collection('friend_links').doc(linkId).set({
      'status': 'accepted',
      'updatedAt': FieldValue.serverTimestamp(),
    }, SetOptions(merge: true));
  }
...
final schemaFriendLinksControllerProvider = Provider<SchemaFriendLinksController>((ref) {
  return SchemaFriendLinksController(ref.watch(schemaFriendFirestoreProvider));
});
