...
class SpeedDatingQueueController extends Notifier<QueueState> {
  ...
  void _listenForActiveSession() {
    ...
    _userSubscription = _firestore
      ...
      if (activeSession != null) {
        debugPrint('âœ… [Queue] Active session found: $activeSession');
  ...
  Future<void> joinQueue(SpeedDatingPreferences preferences) async {
    ...
    try {
      ...
      if (result.data['success'] == true) {
        ...
        debugPrint('âœ… [Queue] Joined queue, waiting for match...');
      } else {
        throw Exception('Failed to join queue');
      }
    } on FirebaseFunctionsException catch (e) {
      debugPrint('âŒ [Queue] Cloud Function error: ${e.code} - ${e.message}');
      state = state.copyWith(
        isLoading: false,
        error: e.message ?? 'Failed to join queue',
      );
    } catch (e) {
      debugPrint('âŒ [Queue] Error joining queue: $e');
      state = state.copyWith(
        isLoading: false,
        error: e.toString(),
      );
  ...
  Future<void> leaveQueue() async {
    ...
    try {
      ...
      debugPrint('âœ… [Queue] Left queue');
    } on FirebaseFunctionsException catch (e) {
      debugPrint('âŒ [Queue] Error leaving queue: ${e.code} - ${e.message}');
      state = state.copyWith(
        isLoading: false,
        error: e.message,
      );
    } catch (e) {
      debugPrint('âŒ [Queue] Error leaving queue: $e');
      state = state.copyWith(
        isLoading: false,
        error: e.toString(),
      );
