...
class SpeedDatingSessionController extends Notifier<SessionState> {
  ...
  Future<void> loadSession(String sessionId) async {
    try {
      ...
      _sessionSubscription = _firestore
        ...
        debugPrint('❌ [Session] Error loading session: $error');
        state = state.copyWith(
          isLoading: false,
  ...
  Future<void> _getAgoraToken(String sessionId) async {
    try {
      ...
      debugPrint('✅ [Session] Got Agora token');
    } on FirebaseFunctionsException catch (e) {
      debugPrint('❌ [Session] Token error: ${e.code} - ${e.message}');
      state = state.copyWith(error: e.message);
    } catch (e) {
      debugPrint('❌ [Session] Error getting token: $e');
  ...
  void _startTimer() {
    ...
    _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (state.session != null) {
        ...
        if (remaining <= 0) {
          timer.cancel();
          debugPrint('⏰ [Session] Time expired');
        }
      }
    });
  }
  ...
  Future<void> makeDecision(String sessionId, String decision) async {
    ...
    try {
      ...
      debugPrint('✅ [Session] Decision submitted: $decision');
    } on FirebaseFunctionsException catch (e) {
      debugPrint('❌ [Session] Decision error: ${e.code} - ${e.message}');
      state = state.copyWith(error: e.message);
      rethrow;
    } catch (e) {
      debugPrint('❌ [Session] Error submitting decision: $e');
      state = state.copyWith(error: e.toString());
      rethrow;
    }
  }
  ...
  Future<void> leaveSession(String sessionId) async {
    try {
      ...
      debugPrint('✅ [Session] Left session');
    } on FirebaseFunctionsException catch (e) {
      debugPrint('❌ [Session] Leave error: ${e.code} - ${e.message}');
      state = state.copyWith(error: e.message);
    } catch (e) {
      debugPrint('❌ [Session] Error leaving session: $e');
      state = state.copyWith(error: e.toString());
    }
  }
}
