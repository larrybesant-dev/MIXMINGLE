...
import 'speed_dating_decision_page.dart';
...
class _SpeedDatingCallPageState extends ConsumerState<SpeedDatingCallPage> {
  ...
  void _listenToServerStatus() {
    ...
    _sessionStatusSubscription = service
      ...
      debugPrint('🔔 [SpeedDating CallPage] Server status: $status (force: $forceDisconnect)');
      ...
      if (status == 'expired' || status == 'abandoned' || forceDisconnect) {
        ...
        if (!_sessionExpiredByServer && mounted) {
          ...
          debugPrint('⏰ [SpeedDating] SESSION EXPIRED BY SERVER - Forcing disconnect');
  ...
  Future<void> _endAgoraCall() async {
    try {
      ...
      debugPrint('📹 [SpeedDating] Ending Agora call...');
    } catch (e) {
  ...
  void _startCountdown() {
    _countdownTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (_remainingSeconds > 0) {
        ...
        debugPrint('⏰ [SpeedDating] Local timer expired, waiting for server confirmation...');

        // Failsafe: if server doesn't respond in 5 seconds, force end
        Future.delayed(const Duration(seconds: 5), () {
          if (!_sessionExpiredByServer && mounted) {
            debugPrint('⚠️ [SpeedDating] Server timeout - forcing local end');
            _onCallEnded();
          }
        });
      }
