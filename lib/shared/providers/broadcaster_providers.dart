...
final broadcasterQueueProvider =
    StreamProvider.family<List<BroadcasterQueue>, String>((ref, roomId) {
...
final currentUserQueueStatusProvider =
    FutureProvider.family<BroadcasterQueue?, String>((ref, roomId) async {
  final service = ref.watch(broadcasterServiceProvider);
  return service.getCurrentUserQueueStatus(roomId);
});

/// Active broadcaster count
final activeBroadcasterCountProvider =
    FutureProvider.family<int, String>((ref, roomId) async {
  final service = ref.watch(broadcasterServiceProvider);
  return service.getActiveBroadcasterCount(roomId);
});

/// Pending broadcast count
final pendingBroadcastCountProvider =
    FutureProvider.family<int, String>((ref, roomId) async {
  final service = ref.watch(broadcasterServiceProvider);
