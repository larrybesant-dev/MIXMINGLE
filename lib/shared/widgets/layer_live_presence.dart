...
class LayerLivePresence extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        _sectionHeader(Icons.broadcast_on_personal, 'Live Presence', const Color(0xFFFFAB00)),
        ...
        if (_isLive) ...[
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: const Color(0xFFFFAB00).withValues(alpha: 0.7)),
              boxShadow: [
                BoxShadow(color: const Color(0xFFFFAB00).withValues(alpha: 0.2), blurRadius: 20),
              ],
            ),
            child: Row(
              children: [
                Container(
                  ...
                  child: const Row(
                    ...
                    children: [
                      _PulsingDot(),
                      SizedBox(width: 6),
                      Text('LIVE NOW', style: TextStyle(color: Colors.black, fontWeight: FontWeight.w900, fontSize: 12)),
                    ],
                  ),
                ),
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: const Text('Join', style: TextStyle(color: Colors.black, fontWeight: FontWeight.w800, fontSize: 13)),
                  ),
                ),
              ],
            ),
        ...
        Container(
          ...
          child: Row(
            children: [
              _statCol(
                p.roomsHostedCount != null ? '${p.roomsHostedCount}' : '0',
                'Rooms Hosted',
                const Color(0xFFFFAB00)),
              _divV(),
              _statCol(
                (p.avgRoomRating != null && p.avgRoomRating! > 0)
        ...
        // Upcoming events
        if (p.eventsHostingCount != null && p.eventsHostingCount! > 0) ...[
          const SizedBox(height: 12),
          Container(
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: const Icon(Icons.event_outlined, color: Color(0xFF00E5CC), size: 20),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(
                      '${p.eventsHostingCount ?? 0} upcoming event${(p.eventsHostingCount ?? 0) > 1 ? 's' : ''}',
                      style: const TextStyle(color: Color(0xFF00E5CC), fontWeight: FontWeight.w700, fontSize: 14),
                    ),
                    const Text('Tap to view & RSVP', style: TextStyle(color: Color(0xFF6B7280), fontSize: 12)),
                  ]),
                ),
                GestureDetector(
                  onTap: onViewEvents,
                  child: const Icon(Icons.arrow_forward_ios, size: 14, color: Color(0xFF00E5CC)),
                ),
              ],
            ),
          ),
  ...
  Widget _statCol(String val, String label, Color color) {
    return Expanded(
      child: Column(children: [
        ...
        Text(label, style: const TextStyle(color: Color(0xFF6B7280), fontSize: 11), textAlign: TextAlign.center),
      ]),
    );
  }
