      ...
      children: [
        ...
        Wrap(
          ...
          children: [
            ...
            if (p.isPremium ?? false) _badge(Icons.workspace_premium_outlined, 'Premium', const Color(0xFFFFAB00)),
            if (p.isCreatorBadge ?? false) _badge(Icons.star_outline_rounded, 'Creator', const Color(0xFFFF6B35)),
            if ((p.isAdultContentEnabled ?? false) && (p.is18PlusVerified ?? false))
        ...
        // Personality prompts
        if ((p.personalityPrompts ?? []).isNotEmpty) ...[
          const SizedBox(height: 16),
          _sectionHeader(Icons.auto_awesome_outlined, 'Vibe Check', const Color(0xFF9B59B6)),
          const SizedBox(height: 8),
          ...List.generate((p.personalityPrompts ?? []).length, (i) => _promptCard(i.toString(), (p.personalityPrompts ?? [])[i].toString())),
        ],
      ],
    );
  }
