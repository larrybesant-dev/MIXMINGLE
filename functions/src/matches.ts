...
function computeMatchScore(userA: any, userB: any): number {
  ...
  const interestOverlap = interestsA.filter((i) => interestsB.includes(i)).length;
 ...
 * Callable function
 */
export const generateUserMatches = onCall({ region: "us-central1" }, async (request) => {
  ...
  try {
    ...
    console.log(`[generateUserMatches] User preferences:`, user.preferences);
    ...
    console.log(`[generateUserMatches] Excluding ${excludeIds.size} users`);
    ...
    if (user.preferences?.ageMin) {
      candidatesQuery = candidatesQuery.where("age", ">=", user.preferences.ageMin);
    }
    if (user.preferences?.ageMax) {
      candidatesQuery = candidatesQuery.where("age", "<=", user.preferences.ageMax);
    }

    const candidatesSnap = await candidatesQuery.limit(200).get();
    console.log(`[generateUserMatches] Found ${candidatesSnap.docs.length} candidates`);
    ...
    console.log(`[generateUserMatches] Generated ${scored.length} matches`);
    ...
    console.log(`[generateUserMatches] Completed for ${uid}: ${scored.length} matches`);
 ...
 */
export const handleLike = onCall({ region: "us-central1" }, async (request) => {
  const uid = request.auth?.uid;
  if (!uid) {
    throw new HttpsError("unauthenticated", "Authentication required");
  }
  ...
  try {
    ...
    const existingLikeSnap = await db.doc(`likes/${uid}/incoming/${targetUserId}`).get();
    ...
    return {
      success: true,
      isMutualLike,
      message: isMutualLike ? "It's a match! 🎉" : "Like recorded successfully",
    };
  } catch (error: any) {
 ...
 */
export const handlePass = onCall({ region: "us-central1" }, async (request) => {
  const uid = request.auth?.uid;
  if (!uid) {
    throw new HttpsError("unauthenticated", "Authentication required");
  }
  ...
  try {
    ...
    // Move to history
    const matchDoc = await db.doc(`matches/${uid}/generated/${targetUserId}`).get();
    if (matchDoc.exists) {
...
export const refreshDailyMatches = onSchedule(
  ...
  async (event) => {
    ...
    try {
      ...
      console.log(`[refreshDailyMatches] Processing ${usersSnap.docs.length} users`);
        ...
        await Promise.all(
          batch.map(async (doc) => {
            try {
              ...
              console.log(`[refreshDailyMatches] Updated ${uid}: ${scored.length} matches`);
            } catch (error) {
              console.error(`[refreshDailyMatches] Error for user ${doc.id}:`, error);
            }
          }),
        );
      }
